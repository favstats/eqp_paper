...
trolley <- read_spss("data/TrolleyExperimentArgumentNew.sav") %>% 
  ...
  mutate(leftright = t2_polalignment) %>% 
  mutate(pol_interest = 5 - t2_polinterest) %>% 
  mutate(church_attendance = 7 - t2_religionchurch) %>% 
  mutate(age = 2018 - as.numeric(t2_yearofbirth)) %>% 
...
mnb <- glm.nb(absolute_change ~ ideal_xtremes + relative_xtremes + general_group_discussion + general_group_information + gender + leftright + pol_interest + church_attendance + age + university, data = nb_dat)
